...
corrThresh = 0.4; % Correlation filter (template match). Keep this LOW (0.1) for noisy data; high (0.6 or 0.7) for clean data
...
lightChannelONlevel = 4.393704461;
...
% file = uigetfile(generalDataRootDirectory,'./*.mat');
% idx = strfind(file,'.');
% ffName=file(idx(1)+1-idx(1):(idx(1)-1));
% 
    ...
    for i=1:length(locs)
        if length(data(max(locs(i)-DBspikeTemplateWidthPre,1): min(locs(i)+DBspikeTemplateWidthPost,length(data(:,1))),1))< DBspikeTemplateWidth
            ...
            if length(DBlocs) > 1
                ...
                if max(corrVal1, corrVal2) > corrThresh
...
% Call GUI to decide on thresholds for good, bad and ugly spike waveforms
...
eval(sprintf('save %s result', [fName '_OpSIN' dt '.mat']));
fName
toc
